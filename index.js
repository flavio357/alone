	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola fdp @${num.split('@')[0]}\nBem vindo ao grupo nao seja GHOST😳🤙 *${mdata.subject}*`
		...
		try {
			...
			switch(command) {
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Mamou o adm errado e perdeu o cargo\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ cade a figurinha fdp ❌')
					reply(mess.wait)
